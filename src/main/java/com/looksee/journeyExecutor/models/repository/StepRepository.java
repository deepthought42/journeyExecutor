...
public interface StepRepository extends Neo4jRepository<Step, Long>{
	...
	@Query("MATCH (d:DomainAuditRecord) with d WHERE id(d)=$domain_audit_id MATCH(d)-[]->(p:PageState{key:$page_state_key}) MATCH (step:Step)-[:STARTS_WITH]->(p:PageState) RETURN step")
